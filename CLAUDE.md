...
## 현재 진행 이슈
- **호가창 스마트 주문** — 2026-04-20 완성: get_orderbook() + _buy_for_account() 지정가 전략
  - 매수잔량 > 매도잔량×1.5 OR 스프레드 > 0.5% → 1호가 지정가 매수
  - 그 외 시장가 유지, 호가 조회 실패 시 시장가 폴백
  - KIS API TR_ID: FHKST01010200
- **Gemma4 마감 보고서** — 2026-04-20 완성: `daily_report_gemma.py` + 크론 21:00 평일
  - proxy_v54.log 파싱 → [REPORT]/매수매도/에러 → Gemma4 분석 → 텔레그램
- **error_monitor 일일 초기화** — 2026-04-20: 자정 에러 트래커 리셋
- **Graphify 코드 그래프** — 2026-04-19 완성: Claude/Qwen 공통 코드 인덱스
  - `graphify-out/GRAPH_REPORT.md` (253줄), `graph.json` (343심볼), `doc/` 파일별 상세
  - git commit 시 post-commit hook으로 자동 재빌드
  - Qwen 태스크에 `[GRAPH CONTEXT]` 자동 prepend (graphify_wrapper.py)
- **태스크 서버 상시 운영** — 2026-04-19: `task-server.service` systemd 등록 완료
...
- **트레이너 실전** — 실전매매 전환 완료 (`44197559-01`, REAL_TRADE=True, 2026-04-17)
...
### 서버보수에이전트 수정사항 (2026-04-19)
- **LM Studio 모델 언로드 자동 감지**: `_is_model_unloaded_resp()` — 빈 choices/error 응답 감지
- **모델 로드 대기**: `_wait_for_model(180)` — TCP 체크가 아닌 `GET /v1/models`로 실제 로드 확인
- **call_qwen() / _call_qwen_direct()**: 모델 언로드 감지 → WoL + 모델 로드 대기 + 자동 재시도
- **태스크 서버 systemd 등록**: `task-server.service` — 부팅 시 자동 시작, 장애 시 자동 복구
