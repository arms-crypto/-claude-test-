...
| **2순위** | 백테스트 실제 활용 (KIS MCP, 전략 YAML 설계) | ⭐⭐⭐ | 2~3일 | ✅ 완료 |
...
### 2순위: 백테스트 활용 ✅ 완료 (2026-04-27)
- `setup_lean_data.sh` 실행 → KRX 4010개 종목 등록, symbol-properties 중복 버그 수정
- 삼성전자·한화엔진·두산에너빌리티 3종목 × trend_period 20/60/120 × 2024~2026 백테스트 완료
- **결과**: 업종별 최적 임계값 상이 → `backtest_summary.json` 저장
  - 반도체(005930): trend_period=120 최적 → 권장 `min_signal=7`
  - 방산(082740): trend_period=60 최적 → 권장 `min_signal=6` (현행 유지)
  - 에너지(034020): trend_period=60 최적 → 권장 `min_signal=6` (현행 유지)
- **Gemma 자율 결정 구조 구현**: `pc_director.py`에 `_collect_backtest_summary()` 추가
  - `system_review()` 장전/장후 리뷰 시 백테스트 데이터 주입 → Gemma가 `min_signal_override` 직접 결정
  - 내일 08:00 장전프리뷰부터 자동 반영 (`pc_director.log`로 확인)
...
- **현황 (2026-04-27)**:
  - 1주차 완료 (43개 단위 테스트, `tests/unit/`)
  - 2주차 완료: `tests/contract/test_kis_schema.py` 19개 계약 테스트 전부 통과
    - `to_add`/`to_update` 어서션 타입 수정 (list of dict → `any(item["code"] == ...)`)
  - 전체 테스트: **62개 통과** (`python3 -m pytest tests/ -q`)
  - 남은 것: 3주차 로그 리플레이, 4주차 CI 파이프라인
  - `/compact` 명령 활용 팁: Claude Code 대화창에서 직접 입력 → 컨텍스트 압축
...
## 버그 수정 이력 (2026-04-27)
- **auto_trader.py 재매수 쿨다운 버그**: SELL_ALL/SELL_PARTIAL 시 `last_trades`에 `"date"` 누락
  - 증상: 동일 종목 하루 10~12회 반복 매매 (효성중공업 11회, 해성디에스 10회 등)
  - 원인: buy 쿨다운 체크 `last.get("date") == today`가 SELL 후 항상 우회됨
  - 수정: 3곳(SELL_ALL×2, SELL_PARTIAL×1) 모두 `"date": today` 추가 → 당일 재매수 차단
