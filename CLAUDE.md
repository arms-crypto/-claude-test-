...
**2패스 코드 리뷰 워크플로우 (핵심):**
- 1패스 태스크: "파일을 read_file로 읽고 코드 인용 목록만 출력. 판정 금지."
- 2패스 태스크: "1패스 인용 목록을 바탕으로 BUG/IMPROVEMENT/UNCERTAIN 판정만."
...
# 자동 블로킹 대기 (완료될 때까지 최대 300초 대기 후 반환) ← 1:1 협업 핵심
curl -s "http://127.0.0.1:8001/wait/{task_id}?timeout=300"
```
