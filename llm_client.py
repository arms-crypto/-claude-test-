...
def _clean_llm_output(text: str) -> str:
    ...
    # Gemma 4 tool call 태그가 최종 출력에 남은 경우 제거
...
_READ_FILE_TOOL = {
    ...
    "function": {
        "name": "read_file",
        "description": "서버 파일 읽기. 코드 검토, 로그 확인, 설정 파일 조회 시 사용. 최대 500줄. 큰 파일은 offset으로 구간을 나눠 읽는다. 프로젝트 파일은 'ai_chat.py'처럼 파일명만, 메모리 파일은 'memory/MEMORY.md' 형식.",
        "parameters": {
            "type": "object",
            "properties": {
                "path":   {"type": "string",  "description": "파일 경로. 예: 'auto_trader.py', 'proxy_v54.log'"},
                "offset": {"type": "integer", "description": "시작 줄 번호 (0-based). 기본 0. 큰 파일 분할 읽기 시 사용."},
                "limit":  {"type": "integer", "description": "읽을 줄 수. 기본 500, 최대 500."}
            },
            "required": ["path"]
        }
    }
...
_MACRO_INDICATORS_TOOL = {
    "type": "function",
    "function": {
        "name": "get_macro_indicators",
        "description": "거시지표 조회: 환율(USD/KRW), 유가(WTI), 미국 10년 금리, 나스닥, KOSPI, KOSDAQ. '환율', '달러', '유가', '금리', '나스닥', '시장 분위기' 등 거시경제 질문 시 호출.",
        "parameters": {
            "type": "object",
            "properties": {},
            "required": []
...
_TOOL_SYSTEM = """나는 Google Gemma 4 기반의 자율 행동 에이전트다.

[핵심 행동 원칙 — 최우선 적용]
- 사용자가 요청하면 즉시 도구를 사용해서 결과를 보여준다. 절대 되묻지 않는다.
- "기사를 주시면", "키워드를 주시면", "URL을 주시면" 같은 응답은 금지다.
- 확인/검증/보고서 요청 → read_file로 코드를 직접 읽어 확인한다. 웹 검색으로 대체 금지.
- 시장/뉴스 요청 → 즉시 web_search 또는 get_news를 호출해서 결과를 보여준다.
- 애매한 요청도 문맥상 가장 합리적인 해석으로 바로 행동한다. 확인 질문 금지.
- 항상 한국어로 답변한다. 모델명은 "Google Gemma 4"다.
...
- 계좌는 2개다: 🔵 트레이너 44197559 (실전매매), 🟡 KY 실전계좌 44384407 (실전매매).
...
- "구현 확인", "코드 확인", "반영됐는지 확인", "보고서" 요청 시 → read_file로 코드를 직접 읽어서 확인한다. 뉴스 검색(web_search)으로 대체하지 않는다.
- 시스템 상태 확인 요청 시 → run_command로 직접 점검한다.
...
14. 환율, 달러, 유가, 금리, 나스닥, 거시경제 지표
- get_macro_indicators를 사용한다.
...
[응답 원칙]
- 정해진 형식 없이 질문의 성격에 맞게 자유롭게 답한다.
- 짧은 질문엔 짧게, 복잡한 분석엔 깊이 있게 — 상황에 따라 스스로 판단한다.
- 보고서 형식·이모지·헤더를 강제하지 않는다. 자연스러운 대화체도 좋다.
- 숫자는 필요할 때만 정리하고, 불필요한 나열은 피한다.
- 투자 판단이 필요하면 직접적으로 의견을 말해도 된다. 단 근거를 함께 제시한다.
- 확인·검증 요청 시 사용자에게 되묻지 말고 도구를 먼저 써서 스스로 확인한다.
...
def _execute_tool_call(tool_name: str, arguments: dict) -> str:
    ...
    if tool_name == "get_macro_indicators":
        logger.info("Ollama tool call: get_macro_indicators()")
    ...
    if tool_name == "query_portfolio":
        ...
        try:
            ...
            logger.info("query_portfolio KIS잔고: 트레이너=%s KY=%s",
                        _kis_cash.get("트레이너 44197559"), _kis_cash.get("KY 실전계좌 44384407"))
        except Exception as _e:
            logger.warning("query_portfolio KIS API 실패: %s", _e)
    ...
    if tool_name == "read_file":
        ...
        logger.info("Ollama tool call: read_file('%s', offset=%d, limit=%d)", path, offset, limit)
        ALLOWED_BASES = (
        ...
        if not any(full.startswith(b) for b in ALLOWED_BASES):
            return "접근 거부: 허용된 디렉토리 외부 파일은 읽을 수 없습니다."
        try:
            ...
            if offset + limit < total:
                footer = f"\n... (총 {total}줄 | 표시: {offset+1}~{offset+len(chunk)}줄 | 다음: offset={offset+limit})"
            else:
                footer = f"\n[끝 — 총 {total}줄]"
    ...
    if tool_name == "write_file":
        return "⛔ write_file 권한 없음: 파일 수정은 Claude Code 또는 관리자가 직접 수행합니다."

    if tool_name == "_write_file_disabled":
        PROJ_BASE = "/home/ubuntu/-claude-test-"
        path = arguments.get("path", "")
...
def call_mistral_only(prompt: str, system: str = _TOOL_SYSTEM, use_tools: bool = True, history_messages: list = None) -> str:
    ...
    def _parse_text_tool_call(content: str):
        ...
        # Gemma 4 native 형식: <|tool_call>call:tool_name{...}<tool_call|>
        # 값 형식: key:<|"|>val<|"|> 또는 key:"val" 또는 key:val
        ...
        if _gemma_m:
            ...
            if tool_name in _KNOWN_TOOLS:
                ...
                for _kv in _re.finditer(
                    r'(\w+):\s*(?:<\|"\|>([^<]*)<\|"\|>|"([^"]*)"|\'([^\']*)\'|(\S+))',
                    raw_args
                ):
                    k = _kv.group(1)
                    # 우선순위: <|"|>val<|"|> → "val" → 'val' → bare val
                    v = (_kv.group(2) if _kv.group(2) is not None else
                         _kv.group(3) if _kv.group(3) is not None else
                         _kv.group(4) if _kv.group(4) is not None else
                         _kv.group(5) or "")
                    try:
                        args[k] = int(v)
    ...
    for attempt in range(1, config.LLM_MAX_RETRY + 1):
        try:
            ...
            for _round in range(6):
                ...
                # 단독 호출(round 0)만 즉시 반환 — 체인 중(round 1+)엔 Gemma가 종합
                if tool_name in _DIRECT_RETURN_TOOLS and _round == 0:
                    return tool_result
                ...
                messages.append({"role": "user", "content": (
                    f"[도구 결과: {tool_name}]\n{tool_result}\n\n"
                    "이 결과를 바탕으로 한국어로 답해줘. "
                    "꼭 필요한 추가 도구가 있으면 1개만 더 호출하고, 아니면 지금 바로 최종 답변을 작성해줘."
                )})
                payload2 = {
                    "model": "google_gemma-4-26b-a4b-it",
                    "messages": messages,
